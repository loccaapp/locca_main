...
public class CheckAndInsertLocation implements RequestHandler<CheckLocationInDTO, LocationOutDTO>  {

    @Override
    public LocationOutDTO handleRequest(CheckLocationInDTO input, Context context) {
        context.getLogger().log("Input: " + input);
                
        OperationResult result = new LocationManager().checkAndInsertLocation(input.user_id, 
																        		input.latitude, 
																        		input.longitude, 
																        		input.radius);
